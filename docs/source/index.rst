...
* MISO version 0.1 (`misopy-0.1.tar.gz`_), Christmas release
...
Human/mouse/fly alternative event annotations files
---------------------------------------------------
...
For example, here is a skipped exon from the set of mouse skipped exons annotation: ::
...
* Mapping for mouse alternative events:

  * `Events to genes mapping for mm9`_ (.zip)

* Mapping for human alternative events:

  * `Events to genes mapping for hg18`_ (.zip)

  * `Events to genes mapping for hg19`_ (.zip)
...
deviation is 15, we would use: ``--paired-end 250 15`` when calling ``run_events_analysis.py`` 
(in addition to the ``--compute-genes-psi`` option).
...
11. **What is the input to the** ``--compare-samples`` **option? Do I have to generate a summary file before using it?** The input to ``--compare-samples`` are two directories (corresponding to the two samples to be compared) that are outputted by a call to ``--compute-genes-psi`` (in ``run_events_analysis.py``). There is no need to generate a summary file before calling ``--compare-samples``, and the output of ``--summarize-samples`` is not used at all when doing a samples comparison. A summary file is just a single-file summary of the information contained in the directory produced by ``--compute-genes-psi`` for a particular sample, made for convenience as a summary of the expression levels of exons/isoforms in that sample. (`back <#faq>`_)
