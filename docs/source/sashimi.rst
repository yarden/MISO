...
The directory containing the event/gene isoform information (in the above example, ``test-data/event-data``) can be any directory generated by indexing a GFF3 file, using the ``index_gff.py`` script that is part of MISO. For more information on indexing, see :ref:`indexing-annotation`.
