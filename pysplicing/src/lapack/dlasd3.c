...
/* Subroutine */ int splicingdlasd3_(integer *nl, integer *nr, integer *sqre, integer 
	...
	integer *idxc, integer *ctot, doublereal *z__, integer *info)
{
    ...
    extern /* Subroutine */ int splicingdlasd4_(integer *, integer *, doublereal *, 
	    ...
	    integer *, integer *), splicingdlacpy_(char *, integer *, integer 
...
/*  -- LAPACK auxiliary routine (version 3.2.2) --   
	...
	*info = -16;
    ...
    if (*info != 0) {
	i__1 = -(*info);
	splicingxerbla_("DLASD3", &i__1, (ftnlen)6);
	return 0;
    }
...
/*     Quick return if possible */
    ...
    if (*k == 1) {
	d__[1] = abs(z__[1]);
	splicingdcopy_(&m, &vt2[vt2_dim1 + 1], ldvt2, &vt[vt_dim1 + 1], ldvt);
	if (z__[1] > 0.) {
	    splicingdcopy_(&n, &u2[u2_dim1 + 1], &c__1, &u[u_dim1 + 1], &c__1);
	} else {
	    i__1 = n;
...
/*     Keep a copy of Z. */
    ...
    splicingdcopy_(k, &z__[1], &c__1, &q[q_offset], &c__1);

/*     Normalize Z. */

    rho = splicingdnrm2_(k, &z__[1], &c__1);
    splicingdlascl_("G", &c__0, &c__0, &rho, &c_b13, k, &c__1, &z__[1], k, info);
    rho *= rho;
...
/*     Find the new singular values. */
    ...
    i__1 = *k;
    for (j = 1; j <= i__1; ++j) {
	splicingdlasd4_(k, &j, &dsigma[1], &z__[1], &u[j * u_dim1 + 1], &rho, &d__[j],
		 &vt[j * vt_dim1 + 1], info);
...
/* L70: */
	...
	temp = splicingdnrm2_(k, &u[i__ * u_dim1 + 1], &c__1);
...
/*     Update the left singular vector matrix. */
    ...
    if (*k == 2) {
	splicingdgemm_("N", "N", &n, k, k, &c_b13, &u2[u2_offset], ldu2, &q[q_offset],
		 ldq, &c_b26, &u[u_offset], ldu);
	goto L100;
    }
    if (ctot[1] > 0) {
	splicingdgemm_("N", "N", nl, k, &ctot[1], &c_b13, &u2[(u2_dim1 << 1) + 1], 
		ldu2, &q[q_dim1 + 2], ldq, &c_b26, &u[u_dim1 + 1], ldu);
	if (ctot[3] > 0) {
	    ktemp = ctot[1] + 2 + ctot[2];
	    splicingdgemm_("N", "N", nl, k, &ctot[3], &c_b13, &u2[ktemp * u2_dim1 + 1]
	...
	splicingdlacpy_("F", nl, k, &u2[u2_offset], ldu2, &u[u_offset], ldu);
    }
    splicingdcopy_(k, &q[q_dim1 + 1], ldq, &u[nlp1 + u_dim1], ldu);
    ktemp = ctot[1] + 2;
    ctemp = ctot[2] + ctot[3];
    splicingdgemm_("N", "N", nr, k, &ctemp, &c_b13, &u2[nlp2 + ktemp * u2_dim1], ldu2,
...
L100:
    i__1 = *k;
    for (i__ = 1; i__ <= i__1; ++i__) {
	temp = splicingdnrm2_(k, &vt[i__ * vt_dim1 + 1], &c__1);
...
/*     Update the right singular vector matrix. */
    ...
    if (*k == 2) {
	splicingdgemm_("N", "N", k, &m, k, &c_b13, &q[q_offset], ldq, &vt2[vt2_offset]
		, ldvt2, &c_b26, &vt[vt_offset], ldvt);
	return 0;
    }
    ktemp = ctot[1] + 1;
    splicingdgemm_("N", "N", k, &nlp1, &ktemp, &c_b13, &q[q_dim1 + 1], ldq, &vt2[
	    vt2_dim1 + 1], ldvt2, &c_b26, &vt[vt_dim1 + 1], ldvt);
    ktemp = ctot[1] + 2 + ctot[2];
    if (ktemp <= *ldvt2) {
	splicingdgemm_("N", "N", k, &nlp1, &ctot[3], &c_b13, &q[ktemp * q_dim1 + 1], 
...
/* L140: */
    ...
    splicingdgemm_("N", "N", k, &nrp1, &ctemp, &c_b13, &q[ktemp * q_dim1 + 1], ldq, &
