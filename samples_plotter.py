...
def load_samples(samples_filename):
    ...
    """
    samples_data, h = csv2array(samples_filename, skiprows=1, raw_header=True)
    samples = []
    log_scores = []
...
if __name__ == '__main__':
    samples_filename = 'sampler_results/two-isoforms/samples/equisized_two_iso.8_simul.10001_iters.500_burnin.10_lag'
    plot_outdir = './'
    print "Outputting plots to: %s" %(plot_outdir)
    samples_data = load_samples(samples_filename)
    samples = samples_data[0]
    log_scores = samples_data[1]
    params = samples_data[2]
    gene = samples_data[3]
    plotter = SamplesPlotter(samples, gene, params, true_psi=0.5)
    plotter.plot(output_dir=plot_outdir, plot_intervals=.95)
